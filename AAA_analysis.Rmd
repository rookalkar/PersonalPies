...
unique_correct_ans <- unique(aaa_study_pie$correct_ans)
...
final_data_aaa_3 %>%
  filter(participant_ID <= 9) %>%
    ggplot(aes(x = y)) +
      ...
      geom_point(aes (y = (correct_ans - flipped_ans), x=correct_ans ), data= subset(aaa_study_pie, participant_ID <= 9 )) +
...
final_aaa_data4 %>%
  filter(participant_ID <= 9) %>%
    ggplot(aes(x = y)) +
      ...
      scale_fill_brewer() +
      facet_wrap( ~ participant_ID, nrow = 3 ) + 
      geom_point(aes (y = (flipped_ans - correct_ans), x=correct_ans ), data= subset(aaa_study_pie, participant_ID <= 9 )) +
...
aaa_pie_data_subset = list(n = length(aaa_study_pie_subset$participant_ID), correct_ans = aaa_study_pie_subset$correct_ans/100, response = aaa_study_pie_subset$flipped_ans/100, segments = c(1,2,4), number_segments = 3, participant = aaa_study_pie_subset$participant_ID, k = max(aaa_study_pie_subset$participant_ID))
